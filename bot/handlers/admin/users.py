...
def user_card_kb(user_id, is_blocked):
    ...
    kb.add(
        InlineKeyboardButton("➕ Выдать валюту", callback_data=f"give_money:{user_id}")
    )
...
async def user_management_actions(call: types.CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if action == "unblock_user":
            ...
            await bot.send_message(user_id, "✅ Ваш доступ восстановлен.")
...
async def process_money_amount(message: types.Message, state: FSMContext):
    ...
    # Передаём ID, а не ORM объект
    await check_achievements(user_id)

    await message.reply(
        f"✅ Выдано <b>{amount}</b> монет пользователю <code>{user_id}</code>",
        parse_mode="HTML"
    )
...
def register_admin_users(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(
        ...
        lambda c: c.data.startswith("give_money")
        or c.data.startswith("block_user")
        or c.data.startswith("unblock_user")
    )
